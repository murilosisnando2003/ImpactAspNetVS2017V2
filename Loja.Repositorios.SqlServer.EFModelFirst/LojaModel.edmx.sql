...
-- --------------------------------------------------
-- Date Created: 05/05/2018 13:47:33
-- Generated from EDMX file: C:\Users\sa4007\Desktop\ImpactAspNetVS2017\Loja.Repositorios.SqlServer.EFModelFirst\LojaModel.edmx
-- --------------------------------------------------
...
-- Creating table 'FornecedoresSet'
CREATE TABLE [dbo].[FornecedoresSet] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Nome] nvarchar(200)  NOT NULL
);
GO
...
-- Creating primary key on [Id] in table 'FornecedoresSet'
ALTER TABLE [dbo].[FornecedoresSet]
ADD CONSTRAINT [PK_FornecedoresSet]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
ADD CONSTRAINT [FK_FornecedoresCategorias_Fornecedores]
    ...
    REFERENCES [dbo].[FornecedoresSet]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO
...
-- Creating foreign key on [Produtos_Id] in table 'ProdutoImagem'
ALTER TABLE [dbo].[ProdutoImagem]
...
-- Creating non-clustered index for FOREIGN KEY 'FK_ProdutosProdutoImagem'
CREATE INDEX [IX_FK_ProdutosProdutoImagem]
ON [dbo].[ProdutoImagem]
    ([Produtos_Id]);
GO
